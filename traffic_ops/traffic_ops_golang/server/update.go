...
func UpdateHandler(w http.ResponseWriter, r *http.Request) {
	...
	if hasConfig_update_time {
		configUpdateTime, err = time.Parse(time.RFC3339, config_update_time)
		if err != nil {
	...
	if hasConfig_apply_time {
		configApplyTime, err = time.Parse(time.RFC3339, config_apply_time)
		if err != nil {
	...
	if hasRevalidate_update_time {
		revalUpdateTime, err = time.Parse(time.RFC3339, revalidate_update_time)
		if err != nil {
	...
	if hasRevalidate_apply_time {
		revalApplyTime, err = time.Parse(time.RFC3339, revalidate_apply_time)
		if err != nil {
